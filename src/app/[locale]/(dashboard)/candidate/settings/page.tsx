...
import { Bell, Shield, Trash2, Lock, Loader2 } from 'lucide-react'
...
const NOTIF_DEFAULTS = { emailApplications: true, emailMessages: true, emailNewJobs: false }
const PRIVACY_DEFAULTS = { profilePublic: true, showInSearch: true, allowContactByHotels: true }
    ...
    <div className="space-y-6 max-w-2xl">
      ...
      <div className="rounded-2xl border border-border bg-card p-6">
        ...
        <div className="divide-y divide-border">
          <ToggleRow label="Application updates" description="When your application status changes"
            checked={notifications.emailApplications} onChange={v => setNotifications(p => ({ ...p, emailApplications: v }))} />
          <ToggleRow label="New messages" description="When you receive a message from a hotel"
            checked={notifications.emailMessages} onChange={v => setNotifications(p => ({ ...p, emailMessages: v }))} />
          <ToggleRow label="New job alerts" description="Weekly digest of new hospitality jobs matching your profile"
            checked={notifications.emailNewJobs} onChange={v => setNotifications(p => ({ ...p, emailNewJobs: v }))} />
        </div>
      ...
      <div className="rounded-2xl border border-border bg-card p-6">
        ...
        <p className="text-xs text-muted-foreground mb-4">Control who can see your profile</p>
        <div className="divide-y divide-border">
          <ToggleRow label="Public profile" description="Allow hotels to find and view your profile"
            checked={privacy.profilePublic} onChange={v => setPrivacy(p => ({ ...p, profilePublic: v }))} />
          <ToggleRow label="Appear in search results" description="Show your profile in hotel candidate searches"
            checked={privacy.showInSearch} onChange={v => setPrivacy(p => ({ ...p, showInSearch: v }))} />
          <ToggleRow label="Allow direct contact" description="Let hotels message you directly without applying"
            checked={privacy.allowContactByHotels} onChange={v => setPrivacy(p => ({ ...p, allowContactByHotels: v }))} />
        </div>
