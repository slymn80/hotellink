...
import {
  Search, Users, Filter, MapPin, Globe, Briefcase,
  Star, MessageSquare, Eye, ChevronRight, Loader2, BadgeCheck,
} from 'lucide-react'
...
const DEPARTMENTS = [
  ...
  'MANAGEMENT', 'SALES_MARKETING', 'ENTERTAINMENT',
]
const DEPARTMENT_LABELS: Record<string, string> = {
  FRONT_OFFICE: 'Front Office', FOOD_BEVERAGE: 'Food & Beverage',
  HOUSEKEEPING: 'Housekeeping', SPA_WELLNESS: 'Spa & Wellness',
  MANAGEMENT: 'Management', SALES_MARKETING: 'Sales & Marketing',
...
export default function HotelCandidatesPage() {
  ...
  useEffect(() => {
    ...
    fetchCandidates()
  }, [debouncedQuery, department, availability, language, page])
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            {total} hospitality professionals available
          </p>
        </div>
      </div>
            ...
            return (
              ...
              >
                <div className="flex items-start gap-3 mb-3">
                  ...
                  <span className={`text-xs font-medium flex-shrink-0 ${availConfig?.color}`}>
                    {availConfig?.label}
                  </span>
                </div>
                ...
                {candidate.languages.length > 0 && (
                  <div className="mb-2 flex flex-wrap gap-1.5">
                    {candidate.languages.slice(0, 4).map((lang) => (
                      <Badge key={lang.language} variant="secondary" size="sm">
                        {lang.language}
                      </Badge>
                    ))}
                  </div>
                ...
                {candidate.skills.length > 0 && (
                  <div className="mb-3 flex flex-wrap gap-1.5">
                    {candidate.skills.slice(0, 3).map((skill) => (
                      <Badge key={skill.skill} variant="outline" size="sm">{skill.skill}</Badge>
                    ))}
                    {candidate.skills.length > 3 && (
                      <Badge variant="ghost" size="sm">+{candidate.skills.length - 3} more</Badge>
                    )}
                  </div>
                )}
