...
import {
  Users, Filter, Star, Eye, MessageSquare, CheckCircle,
  XCircle, Clock, ChevronRight, Loader2, User, Briefcase,
  Globe, MapPin,
} from 'lucide-react'
...
export default function HotelApplicationsPage() {
  ...
  const handleToggleStar = async (appId: string, isStarred: boolean) => {
    try {
      ...
      if (res.ok) {
        setApplications(apps => apps.map(a => a.id === appId ? { ...a, isStarred: !isStarred } : a))
      }
    } catch {
      // silent fail
    }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex gap-2 overflow-x-auto pb-1">
        {['', 'SUBMITTED', 'UNDER_REVIEW', 'SHORTLISTED', 'INTERVIEW_SCHEDULED', 'OFFER_EXTENDED', 'HIRED', 'REJECTED'].map((s) => (
          <button
            key={s}
      ...
      {loading ? (
        ...
        <div className="rounded-2xl border border-border bg-card p-12 text-center">
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            Applications from candidates will appear here once you post jobs.
          </p>
          <Link href={`/${locale}/hotel/jobs`}>
            ...
            return (
              <motion.div
                ...
                className={`rounded-2xl border bg-card p-5 transition-shadow hover:shadow-card ${
                  app.isStarred ? 'border-amber-200 dark:border-amber-800/50' : 'border-border'
                }`}
              >
                <div className="flex items-start gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-start justify-between gap-2 mb-1">
                      <div>
                        <div className="flex items-center gap-2">
                          ...
                          <button
                            onClick={() => handleToggleStar(app.id, app.isStarred)}
                            className={`text-lg transition-colors ${app.isStarred ? 'text-amber-500' : 'text-muted-foreground hover:text-amber-500'}`}
                          >
                            ★
                          </button>
                        </div>
                      ...
                      <Badge variant={config?.variant ?? 'secondary'} size="sm" className="flex-shrink-0">
                    ...
                    <div className="flex flex-wrap items-center gap-3 text-xs text-muted-foreground mb-2">
                      {app.candidate.cityOfResidence && (
                        <span className="flex items-center gap-1">
                          <MapPin className="h-3 w-3" /> {app.candidate.cityOfResidence}
                        </span>
                      )}
                      {app.candidate.yearsOfExperience !== null && (
                        <span className="flex items-center gap-1">
                          <Briefcase className="h-3 w-3" /> {app.candidate.yearsOfExperience}+ years
                        </span>
                      )}
                      <span className="flex items-center gap-1">
                        <Clock className="h-3 w-3" /> Applied {formatRelativeDate(new Date(app.createdAt))}
                      </span>
                    ...
                    <div className="flex flex-wrap gap-1.5 mb-3">
                      {app.candidate.languages.slice(0, 3).map((l) => (
                        <Badge key={l.language} variant="secondary" size="sm">
                          <Globe className="mr-1 h-2.5 w-2.5" /> {l.language}
                        </Badge>
                      ))}
                      {app.candidate.skills.slice(0, 2).map((s) => (
                        <Badge key={s.skill} variant="outline" size="sm">{s.skill}</Badge>
                      ))}
                    </div>

                    {/* Cover letter preview */}
                    {app.coverLetter && (
                      <p className="text-xs text-muted-foreground line-clamp-2 mb-3 italic border-l-2 border-primary/30 pl-2">
                        "{app.coverLetter}"
                      </p>
                    )}
                    ...
                    <div className="flex flex-wrap items-center gap-2">
                      ...
                      {app.status === 'HIRED' && !ratedCandidates.has(app.candidate.id) && (
                      ...
                      {transitions.length > 0 && (
                        <div className="flex gap-1.5">
                          {transitions.slice(0, 2).map((nextStatus) => {
                            ...
                            const isPositive = ['SHORTLISTED', 'INTERVIEW_SCHEDULED', 'INTERVIEW_COMPLETED', 'OFFER_EXTENDED', 'HIRED', 'UNDER_REVIEW'].includes(nextStatus)
                            return (
                              <Button
                                key={nextStatus}
                                variant={isPositive ? 'gradient' : 'outline'}
                                size="sm"
                    ...
                    {ratingAppId === app.id && (
                      <div className="mt-3 rounded-xl border border-border bg-muted/30 p-4 space-y-3">
                        <p className="text-sm font-medium text-foreground">
                          Rate {app.candidate.firstName} {app.candidate.lastName}
                        </p>
                        ...
                        <input type="text" placeholder="Title (optional)" maxLength={120}
                          value={ratingForm.title}
                          onChange={e => setRatingForm(f => ({ ...f, title: e.target.value }))}
                          className="w-full rounded-lg border border-border bg-background px-3 py-2 text-sm focus:border-primary focus:outline-none" />
                        <textarea placeholder="Share your experience... (optional)" rows={3} maxLength={2000}
                        ...
                        <div className="flex justify-end gap-2">
                          ...
                          <Button type="button" size="sm" variant="gradient" loading={submittingRating}
                            onClick={() => handleSubmitRating(app.candidate.id)}>
                            Submit Review
                          </Button>
                        </div>
                      </div>
